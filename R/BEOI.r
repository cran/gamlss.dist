...
BEOI = function (mu.link = "logit", sigma.link = "log", nu.link = "logit") 
{
    ...
    structure(list(family = c("BEOI", "One Inflated Beta"),
        ...
        dldd = function() {      #first derivate log-density respect to sigma 
            ...
            mustart <-digamma(p)-digamma(q) 
            ystart  <- log(y) - log(1 - y)
            dldd <- ifelse((y == 1) , 0, mu * (ystart - mustart)
        ...
        G.dev.incr = function(y, mu, sigma, nu, ...){  # Global deviance
            G.dev.incr <- -2 * dBEOI(y, mu, sigma, nu, log = TRUE)
        },
...
#----------------------------------------------------------------------------------------
########## Density  function of One Inflated Beta ##########
dBEOI = function (y, mu = 0.5, sigma = 1, nu = 0.1, log = FALSE) 
{
    ...
    log.lik <- ifelse(y == 1, log(nu), log(1 - nu) + log.beta)
    if (log == FALSE) 
        fy <- exp(log.lik)
    else fy <- log.lik
    fy
